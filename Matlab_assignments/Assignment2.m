...
HB = [inf*ones(1,nx) inf*ones(1,nx) inf*ones(1,L) inf*ones(1,L)];
%[zopt, fval, exitflag, output, lambdaopt] = linprog(c,[],[],D,d,LB,HB); % If we want, we can verify that we get the same solution
%xopt = zopt(1:nx) - zopt(nx+1:2*nx);
%lambdaopt = lambdaopt.ineqlin

% Dual problem
[lambdaopt,fval,exitflag,output,Zopt] = linprog(-d',D',c'); % Specify the three first input parameters!
xopt = Zopt.ineqlin(1:nx) - Zopt.ineqlin(nx+1:2*nx); % The optimal x is now the lagrangian multiplier of the inequality constraint
